...
TEST(HttpServerNetworkTransportTests, BindNetwork) {
    ...
    std::mutex mutex;
    ASSERT_TRUE(transport.BindNetwork(
        0,
        [&connections, &condition, &mutex](std::shared_ptr<Http::Connection> connection)
        {
    ...
    ASSERT_TRUE(client.Connect(0x7F000001, port));
    {
        ...
        ASSERT_TRUE(condition.wait_for(lock, std::chrono::seconds(1),
                                       [&connections] { return !connections.empty(); }));
    }
}
...
TEST(HttpServerNetworkTransportTests, UnBindNetwork) {
    ...
    SystemUtils::NetworkConnection client;
    ASSERT_FALSE(client.Connect(0x7F000001, port));
}
...
TEST(HttpServerNetworkTransportTests, DataReceivingFromClient) {
    ...
    ASSERT_TRUE(transport.BindNetwork(0,
    ...
    ASSERT_EQ(StringUtils::sprintf("127.0.0.1:%" PRIu16, client.GetBoundPort()),
...
TEST(HttpServerNetworkTransportTests, DataSendingtoClient) {
    ...
    std::mutex mutex;
    std::vector<uint8_t> dataReceived;
    const auto dataReceivedDelegate = [&dataReceived, &mutex,
                                       &condition](const std::vector<uint8_t>& message) {
...
TEST(HttpServerNetworkTransportTests, DataReceivedShouldNotRaceConnectionDelegate) {
    ...
    ASSERT_TRUE(
        transport.BindNetwork(0,
                              [&connections, &condition, &mutex,
                               dataReceivedDelegate](std::shared_ptr<Http::Connection> connection)
                              {
