...
namespace
{
    ...
    struct ConnectionAdapter : public Http::Connection
    {
        ...
        virtual std::string GetPeerId() override {
            return StringUtils::sprintf("%" PRIu8 ".%" PRIu8 ".%" PRIu8 ".%" PRIu8 ":%" PRIu16,
...
namespace HttpNetworkTransport
{
    ...
    bool HttpServerNetworkTransport::BindNetwork(uint16_t port,
                                                 NewConnectionDelegate newConnectionDelegate) {
        return impl_->endpoint.Open(
            [this,
             newConnectionDelegate](std::shared_ptr<SystemUtils::NetworkConnection> newConnection)
            {
            ...
            [this](uint32_t address, uint16_t port, const std::vector<uint8_t>& body)
            {
            ...
            SystemUtils::NetworkEndPoint::Mode::Connection, 0, 0, port);
    }

    uint16_t HttpServerNetworkTransport::GetBoundPort() { return impl_->endpoint.GetBoundPort(); }

    void HttpServerNetworkTransport::ReleaseNetwork() { return impl_->endpoint.Close(); }
}  // namespace HttpNetworkTransport
